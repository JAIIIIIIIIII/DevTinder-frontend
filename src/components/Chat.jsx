...
import createSocketConnection from '../utils/soket';
...
const Chat = () => {
    ...
    const getChat = async () =>{
    try { const res = await axios.get( BASE_URL + "/getchat/" +id,{withCredentials:true});
    ...
    useEffect(() => {
      try {
        ...
        const socket = createSocketConnection(); // Create socket connection
    ...
    const handleClick = async () =>{
      ...
      socket.emit("sendMessage" , {sender:user , receiver:id , message:inputMessage});
  ...
  className=" rounded-l-lg px-3 input-primary w-full max-w-xs  rounded-r-none" />
        <button className="btn btn-primary rounded-l-none" onClick={handleClick}>Send </button> 
        </div>
